...
class Generator(nn.Module):
    def __init__(self, input_channels=512, output_channels=3, up_times=5): # up_times=5 就是256*256
        super().__init__()
        layers = []
        bias_flag = False
        ...
        layers.append(nn.ConvTranspose2d(256,output_channels,kernel_size=3, padding=1, bias=bias_flag))
        PixelNormLayer()
        layers.append(nn.Tanh())
...
class Discriminator_SpectrualNorm(nn.Module):
    def __init__(self, input_channels=512, output_channels=3, up_times=5, bias_flag = False):
        super().__init__()
        layers=[]
