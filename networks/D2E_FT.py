...
class Generator_v3(nn.Module):
    def __init__(self, output_channels=3, image_size=512, first_hidden_dim_ = 512, last_hidden_dim_ =64, another_times=0):
        super().__init__()
        up_times = math.log(image_size,2)- 3 - another_times # 减去前两次 1->2->4， 及最后一次， 方便中间写循环
        bias_flag = False
        ...
        # 1: 
        layers = []
        layers.append(nn.ConvTranspose2d(first_hidden_dim_, first_hidden_dim_, kernel_size=4,stride=1,padding=0,bias=bias_flag)) # 1*1 input -> 4*4
        layers.append(nn.InstanceNorm2d(first_hidden_dim_, affine=False, eps=1e-8))
        layers.append(nn.ReLU())
        setattr(self, "layer%d" % (1), nn.Sequential(*layers))

        # 2: upsamplings, 4x4 -> 8x8 -> 16x16 -> 32*32
        hidden_dim = first_hidden_dim_
        i=2
        while up_times>0:
            if i > (math.log(first_hidden_dim_,2) - math.log(last_hidden_dim_,2)+1):
                layers = []
                layers.append(nn.ConvTranspose2d(hidden_dim, hidden_dim//2, kernel_size=4, stride=2, padding=1 ,bias=bias_flag))
                layers.append(nn.InstanceNorm2d(hidden_dim//2, affine=False, eps=1e-8))
                layers.append(nn.ReLU())
...
class Discriminator_SpectrualNorm_v3(nn.Module):
    def __init__(self, input_channels=3, image_size=512, first_hidden_dim_=64, last_hidden_dim_=512, another_times=0): #新版的Dscale是相对G缩小的倍数
        super().__init__()
        layers=[]
        up_times = math.log(image_size,2)- 3 - another_times
        bias_flag = False
        ...
        # 3:
        layers = [] 
        layers.append(nn.Conv2d(hidden_dim, last_hidden_dim_, kernel_size=4, stride=1, padding=0)) # 4*4 > 1*1
        #layers.append(nn.Conv2d(hidden_dim, last_hidden_dim_, kernel_size=4, stride=2, padding=1)) # 8*8 > 4*4
        setattr(self, "layer%d" % (i), nn.Sequential(*layers))
